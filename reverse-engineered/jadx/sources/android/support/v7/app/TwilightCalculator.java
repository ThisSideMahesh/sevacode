...
class TwilightCalculator {
    ...
    public void calculateTwilight(long j, double d, double d2) {
        ...
        Double.isNaN(d3);
        double dSin2 = dSin + d3 + (Math.sin(2.0f * f2) * 3.4906598739326E-4d) + (Math.sin(f2 * 3.0f) * 5.236000106378924E-6d) + 1.796593063d + 3.141592653589793d;
        double d4 = (-d2) / 360.0d;
        double d5 = f - J0;
        Double.isNaN(d5);
        double dRound = Math.round(d5 - d4) + J0;
        Double.isNaN(dRound);
        double dSin3 = dRound + d4 + (Math.sin(d3) * 0.0053d) + (Math.sin(2.0d * dSin2) * (-0.0069d));
