...
public final class Downsampler {
    ...
    private Bitmap decodeFromWrappedStreams(InputStream inputStream, BitmapFactory.Options options, DownsampleStrategy downsampleStrategy, DecodeFormat decodeFormat, boolean z, int i, int i2, boolean z2, DecodeCallbacks decodeCallbacks) throws IOException {
        ...
        calculateConfig(inputStream, decodeFormat, z3, zIsExifOrientationRequired, options, i5, i6);
            ...
            if (downsampler.shouldUsePool(type)) {
                if (i3 < 0 || i4 < 0 || !z2 || !z4) {
                    ...
                    if (Log.isLoggable(TAG, 2)) {
                        Log.v(TAG, "Calculated target [" + iRound + "x" + iRound2 + "] for source [" + i3 + "x" + i4 + "], sampleSize: " + i7 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                    }
                } else {
                    iRound = i5;
                    iRound2 = i6;
        ...
        if (bitmapDecodeStream != null) {
            ...
            bitmapRotateImageExif = TransformationUtils.rotateImageExif(downsampler.bitmapPool, bitmapDecodeStream, orientation);
